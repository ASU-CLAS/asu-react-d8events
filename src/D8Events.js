...
class EventItemDefault extends Component {
  render() {
    ...
    if (this.props.listNode.time_description != "") {
      ...
      time_section = (
        <>
          <div>{ReactHtmlParser(newTime[0])}</div>
        </>
      );
    }
    ...
    return (
      <div>
        <Card
          ...
          type="event"
          horizontal={true}
          image={this.props.listNode.image_url}
          imageAltText={this.props.listNode.title}
          title={this.props.listNode.title}
          eventFormat="inline"
          eventTime={this.props.listNode.start_date}
...
class D8Events extends Component {
  ...
  render() {
    var results = this.state.displayData.map((thisNode) => ({
      ...
      end_date: thisNode.node.end_date,
      ...
      alias_indexed: thisNode.node["alias-indexed"],
      locations: thisNode.node.locations,
      time_description: thisNode.node.time_description,
    }));
